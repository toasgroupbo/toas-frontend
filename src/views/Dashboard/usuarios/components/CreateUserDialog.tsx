...
import type { CreateUserDto, UpdateUserDto, User } from '@/types/api/users'
...
const CreateUserDialog = ({ open, onClose, onSubmit, isLoading, user, mode = 'create' }: CreateUserDialogProps) => {
  ...
  }, [open, isEditMode, user, reset])

  const handleFormSubmit = (data: CreateUserFormData | UpdateUserFormData) => {
    if (isEditMode) {
      const { password, ...updateData } = data as CreateUserFormData
  ...
  const dynamicRoles = roles?.filter(role => !role.isStatic) || []
    ...
    <Dialog open={open} onClose={handleClose} maxWidth='sm' fullWidth>
      <DialogTitle>
        <Box display='flex' justifyContent='space-between' alignItems='center'>
          <Box>
            <Typography variant='h5'>{isEditMode ? 'Editar Usuario' : 'Crear Nuevo Usuario'}</Typography>
